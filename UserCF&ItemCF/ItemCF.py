    ...
    def calc_movie_sim(self):
        ...
        for user, movies in self.trainSet.items():
            for m1 in movies:
                for m2 in movies:
                    ...
                    weight = 1
                    # 根据用户活跃度进行加权
                    # weight = 1/math.log2(1+len(movies))
                    self.movie_sim_matrix[m1][m2] += weight
        ...
        for m1, related_movies in self.movie_sim_matrix.items():
            ...
            for m2, count in related_movies.items():
                ...
                # 更新最大值
                mx = max(self.movie_sim_matrix[m1][m2],mx)
            # 进行相似度归一化
            for m2, count in related_movies.items():
                self.movie_sim_matrix[m1][m2] /= mx
        print('Calculate movie similarity matrix success!')


    # 针对目标用户U，对其看过的每部电影找到K部相似的电影，并推荐其N部电影
    def recommend(self, user):
        ...
        for movie, rating in watched_movies.items():
            for related_movie, w in sorted(self.movie_sim_matrix[movie].items(), key=itemgetter(1), reverse=True)[:K]:
                ...
                # 排名的依据——>用户对已看电影的评分*推荐电影与该已看电影的相似度(累计)
